...
def main():
    ...
    acl_del = acl_sub.add_parser("delete")
    acl_del.add_argument("id", type=int)
    ...
    c_renew = c_sub.add_parser("renew")
    c_renew.add_argument("id", type=int)
    c_dl = c_sub.add_parser("download")
    c_dl.add_argument("id", type=int)

    c_up = c_sub.add_parser("upload")
    c_up.add_argument("--payload", required=True, help="JSON payload")
    c_val = c_sub.add_parser("validate")
    c_val.add_argument("--payload", required=True, help="JSON payload")
    c_test = c_sub.add_parser("test-http")
    c_test.add_argument("--payload", required=True, help="JSON payload")

    # USER
    u = subparsers.add_parser("user", help="Manage Users")
    u_sub = u.add_subparsers(dest="action", required=True)
    u_sub.add_parser("list")
    u_search = u_sub.add_parser("search")
    ...
    if args.category == "acl":
        if args.action == "list":
            ...
            client.create_access_list(
                {"name": args.name, "satisfy_any": args.satisfy_any, "items": items, "clients": []}
            )
            print("Created.")
        elif args.action == "delete":
            client.delete_access_list(args.id)
            print("Deleted.")
            ...
            if args.items:
                current["items"] = json.loads(args.items)
        ...
        if args.action in ["list", "search"]:
            d = filter_data(
                client.list_items(ep), args.query if args.action == "search" else None, ["domain_names", "forward_host"]
            )
            if not args.json:
                for i in d:
            ...
            client.create_proxy(
                {
                    ...
                    "meta": {"pass_auth_to_upstream": args.pass_auth},
            ...
            if args.domains:
                current["domain_names"] = args.domains
            if args.ip:
                current["forward_host"] = args.ip
            if args.port:
                current["forward_port"] = args.port
            if args.scheme:
                current["forward_scheme"] = args.scheme
            if args.access_list_id is not None:
                current["access_list_id"] = args.access_list_id
            # Toggles
            if args.ssl_forced is not None:
                current["ssl_forced"] = args.ssl_forced
            if args.caching is not None:
                current["caching_enabled"] = args.caching
            if args.block_exploits is not None:
                current["block_exploits"] = args.block_exploits
            if args.websockets is not None:
                current["allow_websocket_upgrade"] = args.websockets
            if args.http2 is not None:
                current["http2_support"] = args.http2
            if args.hsts is not None:
                current["hsts_enabled"] = args.hsts
            if args.pass_auth is not None:
                if not current.get("meta"):
                    current["meta"] = {}
        ...
        if args.action == "list":
            ...
            client.create_redirect(
                {
                    "domain_names": args.domains,
                    "forward_domain_name": args.forward_domain,
                    "forward_http_code": args.http_code,
                    "forward_scheme": args.scheme,
            ...
            if args.domains:
                current["domain_names"] = args.domains
            if args.forward_domain:
                current["forward_domain_name"] = args.forward_domain
            if args.http_code:
                current["forward_http_code"] = args.http_code
            if args.scheme:
                current["forward_scheme"] = args.scheme
        ...
        if args.action == "list":
            ...
            client.create_stream(
                {
                    ...
                    "forwarding_host": args.forward_host,
                    "forwarding_port": args.forward_port,
                    "tcp_forwarding": args.tcp,
                    "udp_forwarding": args.udp,
                    "certificate_id": 0,
                    "meta": {},
                }
            )
            ...
            if args.incoming_port:
                current["incoming_port"] = args.incoming_port
        ...
        if args.action in ["list", "search"]:
            d = filter_data(
                client.list_items(ep), args.query if args.action == "search" else None, ["domain_names", "nice_name"]
            )
            if not args.json:
                for i in d:
                    i["domains"] = "\n".join(i["domain_names"])
