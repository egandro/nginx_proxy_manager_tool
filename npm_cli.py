...
# Add plugins/module_utils to path so we can share code with Ansible
...
try:
    ...
    print(f"Ensure 'plugins/module_utils/npm_api.py' exists.")
    sys.exit(1)
# --------------------------------
...
def main():
    ...
    # --- PROXY ---
    p_parser = subparsers.add_parser("proxy", help="Manage Proxy Hosts")
    p_subs = p_parser.add_subparsers(dest="action", required=True)
    p_subs.add_parser("list")

    p_search = p_subs.add_parser("search")
    p_search.add_argument("query")

    p_create = p_subs.add_parser("create")
    ...
    for act in ["delete", "enable", "disable"]:
        p = p_subs.add_parser(act)
        p.add_argument("id", type=int)

    # --- REDIRECT ---
    r_parser = subparsers.add_parser("redirect", help="Manage Redirection Hosts")
    r_subs = r_parser.add_subparsers(dest="action", required=True)
    r_subs.add_parser("list")

    r_create = r_subs.add_parser("create")
    ...
    for act in ["delete", "enable", "disable"]:
        p = r_subs.add_parser(act)
        p.add_argument("id", type=int)

    # --- 404 ---
    d_parser = subparsers.add_parser("404", help="Manage 404 Hosts")
    d_subs = d_parser.add_subparsers(dest="action", required=True)
    d_subs.add_parser("list")
    d_create = d_subs.add_parser("create")
    d_create.add_argument("--domains", nargs='+', required=True)

    for act in ["delete", "enable", "disable"]:
        p = d_subs.add_parser(act)
        p.add_argument("id", type=int)

    # --- STREAMS ---
    s_parser = subparsers.add_parser("stream", help="Manage Streams")
    s_subs = s_parser.add_subparsers(dest="action", required=True)
    s_subs.add_parser("list")

    s_create = s_subs.add_parser("create")
    ...
    for act in ["delete", "enable", "disable"]:
        p = s_subs.add_parser(act)
        p.add_argument("id", type=int)

    # --- CERTIFICATES ---
    c_parser = subparsers.add_parser("cert", help="Manage Certificates")
    c_subs = c_parser.add_subparsers(dest="action", required=True)
    c_subs.add_parser("list")
    c_subs.add_parser("providers")

    c_search = c_subs.add_parser("search")
    c_search.add_argument("query")

    c_http = c_subs.add_parser("create-http")
    c_http.add_argument("--domains", nargs='+', required=True)
    c_http.add_argument("--email", required=True)

    c_dns = c_subs.add_parser("create-dns")
    c_dns.add_argument("--domains", nargs='+', required=True)
    c_dns.add_argument("--email", required=True)
    ...
    c_del = c_subs.add_parser("delete")
    c_del.add_argument("id", type=int)
    c_renew = c_subs.add_parser("renew")
    c_renew.add_argument("id", type=int)

    # --- USERS ---
    u_parser = subparsers.add_parser("user", help="Manage Users")
    u_subs = u_parser.add_subparsers(dest="action", required=True)
    u_subs.add_parser("list")

    u_search = u_subs.add_parser("search")
    u_search.add_argument("query")

    u_create = u_subs.add_parser("create")
    ...
    u_del = u_subs.add_parser("delete")
    u_del.add_argument("id", type=int)
    ...
    # Init with defaults (will load from env/json)
    client = NPMClient()
    ...
    if args.category == "proxy":
        ...
        if args.action in ["list", "search"]:
            ...
            client.delete_item(ep, args.id)
            print("Deleted.")
        elif args.action in ["enable", "disable"]:
            client.toggle_enable(ep, args.id, args.action)
    ...
    # CERT
        ...
        if args.action in ["list", "search"]:
            ...
            client.create_cert({
                "domain_names": args.domains, "provider": "letsencrypt",
                "meta": {"dns_challenge": False, "letsencrypt_email": args.email, "agree_tos": True}
            })
            print("Requested.")
            ...
            client.create_cert({
                "domain_names": args.domains, "provider": "letsencrypt",
                "meta": {"dns_challenge": True, "dns_provider": args.provider, "dns_provider_credentials": creds,
                         "letsencrypt_email": args.email, "propagation_seconds": args.propagation, "agree_tos": True}
            })
            print("Requested.")
        elif args.action == "delete":
            client.delete_item(ep, args.id)
            ...
            client.session.post(f"{client.base_url}/{ep}/{args.id}/renew")
            print("Renew Requested.")
