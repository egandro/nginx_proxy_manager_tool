    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            users=dict(
                ...
                elements="dict",
                options=dict(
                    username=dict(type="str", required=True), password=dict(type="str", required=True, no_log=True)
                ),
                default=None,
            ),
        ),
    ...
    # 3. Create/Update Logic
        ...
        if module.params["users"] is not None:
            for u in module.params["users"]:
                desired_items.append({"username": u["username"], "password": u["password"]})
        ...
        payload = {"name": name, "satisfy_any": module.params["satisfy_any"], "items": desired_items, "clients": []}
        ...
        if not existing:
            ...
            # So, if 'users' is defined, we effectively always update unless we implement a complex hash check
            # (not possible here).
            if module.params["users"] is not None:
                ...
                # To reduce noise, one could check if usernames are identical and skip, but that prevents password
                # rotation.
